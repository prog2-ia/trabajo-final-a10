...
class Usuario(Persona):
    def __init__(self, dni: str, nombre: str, apellido: str, edad: int) -> None:
        ...
        self.__prestamos: list = [] # Lista de los libros que tiene el usuario en posesión porque la biblioteca se los ha prestado

    def prestar_libro(self, libro) -> None: # Método para solicitar un libro a la biblioteca
        if libro.disponible: # Verifica que el libro esté disponible (True)
            libro.prestar() # Se ejecuta el método de libro_local
            nuevo_prestamo: Prestamos = Prestamos(self, libro)
            self.__prestamos.append(nuevo_prestamo)
        else: # Si disponible = False no se puede solicitar
            print('No se puede prestar el libro.')

    def devolver_libro(self, libro) -> None: # Método para que el usuario devuelva un libro a la biblioteca
